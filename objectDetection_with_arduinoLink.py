...
# setting up for arduino link (essentially the same thing in the setup block for any arduino program)
serial_inst = serial.Serial()
serial_inst.baudrate = 9600
# THIS MUST BE ADJUSTED DEPENDING ON THE PORT YOUR ARE USING
serial_inst.port = "WRITE PORT HERE"
serial_inst.open()
    ...
    while True:
        ...
        if show_angle and N > 1:  # if there's only one "region" of mask, then no purple is detected
            ...
            for point in points:    # iterate through center points
                x, y, z = get_ray(point, "matrix0.txt")[0]  # see function
        ...
        # movement control based on object detection
        if N > 1:
            # forward command
            serial_inst.write('w'.encode('utf-8'))
        else:
            # stop command
            serial_inst.write('0'.encode('utf-8'))
...
def get_ray(point, filename="matrix0.txt"):
